...
class RxToRACOptionalTests: XCTestCase {
    ...
    func test_Next() {
        for count in [1, 2, 10] {
            let arrayOfOptionals = Array(0..<count).map { "\($0)" as NSString }.flatMap { [$0, nil] }
            let observable: Observable<NSString?> = Observable.from(arrayOfOptionals)
            let values = expectation(description: "value")
            values.expectedFulfillmentCount = count * 2
            var count = 0
    ...
    func test_Completed() {
        let observable: Observable<NSString?> = Observable<NSString?>.empty()
    ...
    func test_Error() {
        let observable: Observable<NSString?> = Observable<NSString?>.error(NSError(domain: "Error", code: 2, userInfo: nil))
    ...
    func test_Dispose() {
        ...
        let observable: Observable<NSString?> = Observable<NSString?>.create { observer in
